...
<%
    ...
    try{
        ...
        PreparedStatement ps= conn.prepareStatement("update story set status='Publish',date_modified=? where s_id=?");
        ps.setDate(1,date);
        ps.setString(2,sid);
        
        int rn= ps.executeUpdate();
        if(rn>0){
            String qry1 = "update parts set status='Publish' where s_id='"+sid+"' and status!='Draft'";
